...
class ChatbotAssistant:
    ...
    def get_response(self, message: str, session_id: str, language: str = 'uk') -> str:
        """Get chatbot response using OpenAI ChatCompletion with IZI.SOFT sales prompt"""
        if not self.client:
        ...
        try:
            ...
            # Prepare IZI.SOFT chatbot request
            chatbot_request = {
                "message": message,
                "language": language,
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False, encoding='utf-8') as f:
                json.dump(chatbot_request, f, ensure_ascii=False, indent=2)
                temp_file = f.name
            ...
            try:
                ...
                current_app.logger.info(f"Running chatbot script from: {script_dir}")
                current_app.logger.info(f"Full script path: {script_path}")
                current_app.logger.info(f"Temp file: {temp_file}")
                
                # Use full paths for everything to avoid any path resolution issues
                python_executable = sys.executable
                current_app.logger.info(f"Using Python: {python_executable}")
                
                # Use a direct approach that avoids encoding issues
                import codecs
                ...
                try:
                    ...
                    current_app.logger.error(f"Error running chatbot process: {str(e)}")
                    result = type('Result', (), {
                        'returncode': 1,
                        'stdout': '',
                ...
                if result.returncode == 0:
                    try:
                        ...
                        if 'error' in response_data:
                            current_app.logger.error(f"OpenAI IZI chatbot API error: {response_data['error']}")
                    ...
                    current_app.logger.error(f"OpenAI IZI chatbot script error: {result.stderr}")
            ...
            current_app.logger.error(f"Error getting IZI.SOFT chatbot response: {str(e)}")
            return self._get_fallback_response(language)
...
Reply in English, be friendly and helpful. Try to direct the conversation towards purchasing relevant products."""
    ...
    def _direct_openai_call(self, message: str, language: str) -> str:
        """Make a direct OpenAI API call as a fallback"""
        try:
            # Simple prompt to avoid encoding issues
